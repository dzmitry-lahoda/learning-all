...
namespace MatrixExtensions.Manipulation
{
    ...
    /// </summary>
    [Pure]
    public static class NumericArrayManipulationExtensions
    {
        ...
        public static NumericBroad[] Append(this NumericBroad[] vector, NumericNarrow[] right)
        {
            Contract.Requires<ArgumentNullException>(vector != null);
            Contract.Ensures(Contract.Result<NumericBroad[]>().Length == vector.Length + right.Length);
            var appendLength = right.Length;
            if (appendLength == 0)
            {
                return (NumericBroad[])vector.Clone();
            }
            var length = vector.Length;
            var newLength = length + appendLength;
            var newVector = new NumericBroad[newLength];
            Do.Invoke(
        ...
        public static NumericBroad[,] Stack(this NumericBroad[] vector, NumericNarrow[] bottom)
        {
            ...
            Contract.Ensures(Contract.Result<NumericBroad[,]>().ColumnCount() == vector.Length);
            Contract.Ensures(Contract.Result<NumericBroad[,]>().RowCount() == 2);
            var length = vector.Length;
            var newMatrix = new NumericBroad[2, length];
            if (length == 0)
            {
                return newMatrix;
            }
        ...
        public static NumericBroad[,] Append(this NumericBroad[] vector, NumericNarrow[,] right)
        {
            ...
            Contract.Requires<ArgumentException>(vector.Length == right.RowCount(), "right.RowCount()");
            Contract.Ensures(Contract.Result<NumericBroad[,]>().ColumnCount() == right.ColumnCount() + 1);
            Contract.Ensures(Contract.Result<NumericBroad[,]>().RowCount() == right.RowCount());
            var rowCount = vector.Length;
            var columnCount = right.ColumnCount() + 1;
        ...
        {
            Contract.Requires<ArgumentNullException>(bottom != null, "bottom");
            Contract.Requires<ArgumentException>(vector.Length == bottom.ColumnCount(), "bottom.ColumnCount()");
            Contract.Ensures(Contract.Result<NumericBroad[,]>().ColumnCount() == bottom.ColumnCount());
            Contract.Ensures(Contract.Result<NumericBroad[,]>().RowCount() == bottom.RowCount() + 1);
            var columntCount = vector.Length;
            var rowCount = bottom.RowCount() + 1;
            var result = new NumericBroad[rowCount, columntCount];
            Do.Invoke(
