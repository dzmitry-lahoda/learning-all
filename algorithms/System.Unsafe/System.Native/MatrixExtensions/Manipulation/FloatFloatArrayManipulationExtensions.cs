...
namespace MatrixExtensions.Manipulation
{
    ...
    /// </summary>
    [Pure]
    public static class FloatFloatArrayManipulationExtensions
    {
        ...
        public static float[] Append(this float[] vector, float[] right)
        {
            Contract.Requires<ArgumentNullException>(vector != null);
            Contract.Ensures(Contract.Result<float[]>().Length == vector.Length + right.Length);
            ...
            var newVector = new float[newLength];
            Do.Invoke(
        ...
        public static float[,] Stack(this float[] vector, float[] bottom)
        {
            ...
            Contract.Ensures(Contract.Result<float[,]>().GetLength(1) == vector.Length);
            Contract.Ensures(Contract.Result<float[,]>().GetLength(0) == 2);
            var length = vector.Length;
            var newMatrix = new float[2, length];
            if (length == 0)
            {
                return newMatrix;
            }
        ...
        public static float[,] Append(this float[] vector, float[,] right)
        {
            ...
            Contract.Ensures(Contract.Result<float[,]>().GetLength(1) == right.GetLength(1) + 1);
            Contract.Ensures(Contract.Result<float[,]>().GetLength(0) == right.GetLength(0));
        ...
        public static float[,] Stack(this float[] vector, float[,] bottom)
        {
            ...
            Contract.Ensures(Contract.Result<float[,]>().GetLength(1) == bottom.GetLength(1));
            Contract.Ensures(Contract.Result<float[,]>().GetLength(0) == bottom.GetLength(0) + 1);
            var columntCount = vector.Length;
            var rowCount = bottom.GetLength(0) + 1;
            var result = new float[rowCount, columntCount];
            Do.Invoke(
