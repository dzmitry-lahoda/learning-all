...
namespace MatrixExtensions.Manipulation
{
    ...
    /// </summary>
    [Pure]
    public static class LongLongArrayManipulationExtensions
    {
        ...
        public static long[] Append(this long[] vector, long[] right)
        {
            Contract.Requires<ArgumentNullException>(vector != null);
            Contract.Ensures(Contract.Result<long[]>().Length == vector.Length + right.Length);
            var appendLength = right.Length;
            if (appendLength == 0)
            {
                return (long[])vector.Clone();
            }
            var length = vector.Length;
            var newLength = length + appendLength;
            var newVector = new long[newLength];
            Do.Invoke(
        ...
        public static long[,] Stack(this long[] vector, long[] bottom)
        {
            ...
            Contract.Ensures(Contract.Result<long[,]>().GetLength(1) == vector.Length);
            Contract.Ensures(Contract.Result<long[,]>().GetLength(0) == 2);
            var length = vector.Length;
            var newMatrix = new long[2, length];
            if (length == 0)
            {
                return newMatrix;
            }
        ...
        public static long[,] Append(this long[] vector, long[,] right)
        {
            ...
            Contract.Ensures(Contract.Result<long[,]>().GetLength(1) == right.GetLength(1) + 1);
            Contract.Ensures(Contract.Result<long[,]>().GetLength(0) == right.GetLength(0));
            var rowCount = vector.Length;
            var columnCount = right.GetLength(1) + 1;
            var result = new long[rowCount, columnCount];
            Do.Invoke(
        ...
        public static long[,] Stack(this long[] vector, long[,] bottom)
        {
            ...
            Contract.Ensures(Contract.Result<long[,]>().GetLength(1) == bottom.GetLength(1));
            Contract.Ensures(Contract.Result<long[,]>().GetLength(0) == bottom.GetLength(0) + 1);
            var columntCount = vector.Length;
            var rowCount = bottom.GetLength(0) + 1;
            var result = new long[rowCount, columntCount];
            Do.Invoke(
