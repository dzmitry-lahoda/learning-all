...
namespace MatrixExtensions.Manipulation
{
    ...
    /// </summary>
    [Pure]
    public static class DecimalDecimalArray2DSimpleManipulationExtensions
    {
        ...
        public static decimal[] ToColumnWiseArray(this decimal[,] matrix)
        {
            Contract.Ensures(Contract.Result<decimal[]>().Length == matrix.GetLength(1) * matrix.GetLength(0));
            var rowCount = matrix.GetLength(0);
            var columnCount = matrix.GetLength(1);
            var newVector = new decimal[rowCount * columnCount];
        ...
        /// </pre></example>
        /// <returns>The new vector containing the matrix's elements.</returns>
        public static decimal[] ToRowWiseArray(this decimal[,] matrix)
        {
            Contract.Ensures(Contract.Result<decimal[]>().Length == matrix.GetLength(1) * matrix.GetLength(0));
            var rowCount = matrix.GetLength(0);
            var columnCount = matrix.GetLength(1);
