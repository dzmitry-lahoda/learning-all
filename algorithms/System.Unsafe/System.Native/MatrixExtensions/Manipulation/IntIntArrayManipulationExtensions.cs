...
namespace MatrixExtensions.Manipulation
{
    ...
    /// </summary>
    [Pure]
    public static class IntIntArrayManipulationExtensions
    {
        ...
        public static int[] Append(this int[] vector, int[] right)
        {
            Contract.Requires<ArgumentNullException>(vector != null);
            Contract.Ensures(Contract.Result<int[]>().Length == vector.Length + right.Length);
            ...
            var newVector = new int[newLength];
            Do.Invoke(
        ...
        public static int[,] Stack(this int[] vector, int[] bottom)
        {
            ...
            Contract.Ensures(Contract.Result<int[,]>().GetLength(1) == vector.Length);
            Contract.Ensures(Contract.Result<int[,]>().GetLength(0) == 2);
            var length = vector.Length;
            var newMatrix = new int[2, length];
            if (length == 0)
            {
                return newMatrix;
            }
        ...
        public static int[,] Append(this int[] vector, int[,] right)
        {
            ...
            Contract.Ensures(Contract.Result<int[,]>().GetLength(1) == right.GetLength(1) + 1);
            Contract.Ensures(Contract.Result<int[,]>().GetLength(0) == right.GetLength(0));
        ...
        public static int[,] Stack(this int[] vector, int[,] bottom)
        {
            ...
            Contract.Ensures(Contract.Result<int[,]>().GetLength(1) == bottom.GetLength(1));
            Contract.Ensures(Contract.Result<int[,]>().GetLength(0) == bottom.GetLength(0) + 1);
            var columntCount = vector.Length;
            var rowCount = bottom.GetLength(0) + 1;
            var result = new int[rowCount, columntCount];
            Do.Invoke(
