    ...
    public static class GenericArrayManipulationExtensions
    {
        ...
        public static T[] Shift<T>(this T[] vector, int count) where T : new()
        {
            ...
            if (count >= 0)
            {
                ...
                Parallel.Invoke(
                    () => Parallel.For(0, count, i => newVector[i] = new T()),
                    () => Parallel.For(count, length, i => newVector[i] = vector[i - count])
                    );
            }
            else
            {
                Parallel.Invoke(
                    () => Parallel.For(0, length + count, i => newVector[i] = vector[i - count]),
                    () => Parallel.For(length + count, length, i => newVector[i] = new T())
                    );
            }
            return newVector;
        }
        ...
        public static T[] CircularShift<T>(this T[] vector, int count)
        {
            ...
            if (count >= 0)
            {
                Parallel.Invoke(
                    ...
                    () => Parallel.For(0, count, i => newVector[i] = vector[count + i+1]),
                    () => Parallel.For(count, length, i => newVector[i] = vector[i - count])
                    );
            }
            else
            {
                Parallel.Invoke(
                    () => Parallel.For(0, length + count, i => newVector[i] = vector[i-count]),
                    () => Parallel.For(length + count, length, i => newVector[i] = vector[i + count+1])
                    );
            }
            return newVector;
        }
