...
namespace MatrixExtensions.Manipulation
{
    ...
    /// </summary>
    [Pure]
    public static class DoubleDoubleArray2DSimpleManipulationExtensions
    {
        ...
        public static double[] ToColumnWiseArray(this double[,] matrix)
        {
            Contract.Ensures(Contract.Result<double[]>().Length == matrix.GetLength(1) * matrix.GetLength(0));
            var rowCount = matrix.GetLength(0);
            var columnCount = matrix.GetLength(1);
            var newVector = new double[rowCount * columnCount];
        ...
        /// </pre></example>
        /// <returns>The new vector containing the matrix's elements.</returns>
        public static double[] ToRowWiseArray(this double[,] matrix)
        {
            Contract.Ensures(Contract.Result<double[]>().Length == matrix.GetLength(1) * matrix.GetLength(0));
            var rowCount = matrix.GetLength(0);
            var columnCount = matrix.GetLength(1);
