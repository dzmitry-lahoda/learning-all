...
namespace MatrixExtensions.Manipulation
{
    ...
    /// </summary>
    [Pure]
    public static class DoubleDoubleArrayManipulationExtensions
    {
        ...
        public static double[] Append(this double[] vector, double[] right)
        {
            Contract.Requires<ArgumentNullException>(vector != null);
            Contract.Ensures(Contract.Result<double[]>().Length == vector.Length + right.Length);
            ...
            var newVector = new double[newLength];
            Do.Invoke(
        ...
        public static double[,] Stack(this double[] vector, double[] bottom)
        {
            ...
            Contract.Ensures(Contract.Result<double[,]>().GetLength(1) == vector.Length);
            Contract.Ensures(Contract.Result<double[,]>().GetLength(0) == 2);
            var length = vector.Length;
            var newMatrix = new double[2, length];
            if (length == 0)
            {
                return newMatrix;
            }
        ...
        public static double[,] Append(this double[] vector, double[,] right)
        {
            ...
            Contract.Ensures(Contract.Result<double[,]>().GetLength(1) == right.GetLength(1) + 1);
            Contract.Ensures(Contract.Result<double[,]>().GetLength(0) == right.GetLength(0));
        ...
        public static double[,] Stack(this double[] vector, double[,] bottom)
        {
            ...
            Contract.Ensures(Contract.Result<double[,]>().GetLength(1) == bottom.GetLength(1));
            Contract.Ensures(Contract.Result<double[,]>().GetLength(0) == bottom.GetLength(0) + 1);
            var columntCount = vector.Length;
            var rowCount = bottom.GetLength(0) + 1;
            var result = new double[rowCount, columntCount];
            Do.Invoke(
